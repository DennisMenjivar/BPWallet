...
export class HomePage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public _auxiliarService: AuxiliarService, public toastCtrl: ToastController, public loadingCtrl: LoadingController, public database: DatabaseProvider, private alertCtrl: AlertController, private modal: ModalController) {
  ...
  activeModal() {
    let profileModal = this.modal.create(LoginPage, { userId: 8675309 });
    profileModal.present();
  }

  validateUserLicence() {
    this.database.validateUserLicence().then((data) => {
      if (data == null) {
        this.activeModal();
      }
    })
  ...
  validateLimitMessageBox(message: string) {
    let alert = this.alertCtrl.create({
      title: "ADVERTENCIA",
      message: message,
      buttons: [
